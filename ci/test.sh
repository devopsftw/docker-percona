...
CONT_56=$(docker run -e "CONSUL_JOIN=$CONSUL_IP" -e "CONSUL_DC=circle" -e "CONSUL_SERVICE_NAME=prc56" -e "MYSQL_ROOT_PASSWORD=test" devopsftw/percona:5.6)
CONT_57=$(docker run -e "CONSUL_JOIN=$CONSUL_IP" -e "CONSUL_DC=circle" -e "CONSUL_SERVICE_NAME=prc57" -e "MYSQL_ROOT_PASSWORD=test" devopsftw/percona:5.7)
